...
public class ColorShader implements ApplicationListener {
	...
	final String FRAG = 
			...
			"	tcp.r = smoothstep(0.0,1.0,st.x);\n" +
			"	tcp.g = sin(st.x*PI);\n" +
			"	tcp.b = pow(st.x,1.5);\n" +
			
			"	color.rgb = mix(colorA,colorB,tcp);\n" +
			"	color.rgb = mix(color.rgb,vec3(1.0,0.0,0.0),plot(st,tcp.r));\n" +
			"	color.rgb = mix(color.rgb,vec3(0.0,1.0,0.0),plot(st,tcp.g));\n" +
			"	color.rgb = mix(color.rgb,vec3(0.0,0.0,1.0),plot(st,tcp.b));\n" +
